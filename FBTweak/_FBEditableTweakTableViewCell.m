...
@implementation _FBEditableTweakTableViewCell {
...
- (void)setTweak:(id<FBEditableTweak>)tweak {
  _tweak = tweak;

  self.textLabel.text = tweak.name;

  FBTweakValue value = self.tweak.currentValue ?: self.tweak.defaultValue;
  
  _FBTweakTableViewCellMode mode = _FBTweakTableViewCellModeNone;
  if ([self.tweak.possibleValues isKindOfClass:[NSDictionary class]]) {
...
- (void)_updateMode:(_FBTweakTableViewCellMode)mode
{
  ...
  if (_mode == _FBTweakTableViewCellModeBoolean) {
    ...
    _stepper.hidden = NO;
    if (self.tweak.stepValue) {
      _stepper.stepValue = [self.tweak.stepValue floatValue];
    } else {
      _stepper.stepValue = 1.0;
    }

    if (self.tweak.minimumValue != nil) {
      _stepper.minimumValue = [self.tweak.minimumValue longLongValue];
    } else {
      _stepper.minimumValue = [self.tweak.defaultValue longLongValue] / 10.0;
    }

    if (self.tweak.maximumValue != nil) {
      _stepper.maximumValue = [self.tweak.maximumValue longLongValue];
    } else {
      _stepper.maximumValue = [self.tweak.defaultValue longLongValue] * 10.0;
    }
    ...
    if (self.tweak.minimumValue != nil) {
      _stepper.minimumValue = [self.tweak.minimumValue doubleValue];
    } else if ([self.tweak.defaultValue doubleValue] == 0) {
      _stepper.minimumValue = -1;
    } else {
      _stepper.minimumValue = [self.tweak.defaultValue doubleValue] / 10.0;
    }
    
    if (self.tweak.maximumValue != nil) {
      _stepper.maximumValue = [self.tweak.maximumValue doubleValue];
    } else if ([self.tweak.defaultValue doubleValue] == 0) {
      _stepper.maximumValue = 1;
    } else {
      _stepper.maximumValue = [self.tweak.defaultValue doubleValue] * 10.0;
    }
...
- (void)_updateValue:(FBTweakValue)value primary:(BOOL)primary write:(BOOL)write
{
  ...
  if (_mode == _FBTweakTableViewCellModeBoolean) {
    ...
    if (primary) {
      self.detailTextLabel.text = self.tweak.possibleValues[value];
    }
  } else if (_mode == _FBTweakTableViewCellModeArray) {
    if (primary) {
